...
async def usage_me(
    ...
    # DB fallback: Redis config expired (5min TTL) → re-read from DB and re-cache
    if budget_config_raw is None and session_factory is not None:
        try:
            ...
            async with session_factory() as db_session:
                ...
                if cfg:
                    ...
                    budget_usage_raw = (
                        await redis.get(f"budget:user:{{{user_id}}}:{period}")
                        if redis is not None
                        else None
                    )
                    used = Decimal(budget_usage_raw.decode() if budget_usage_raw else "0")
                    ...
                    # Re-cache to Redis so subsequent calls don't hit DB
                    if redis is not None:
                        await redis.set(
