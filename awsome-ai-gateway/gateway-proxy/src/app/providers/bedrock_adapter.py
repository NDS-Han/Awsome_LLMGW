...
class BedrockAdapter(ProviderAdapter):
    ...
    @staticmethod
    def _extract_bearer_token_region(token: str) -> str | None:
        """Bearer token 의 X-Amz-Credential 에서 region 을 추출한다.

        Bedrock short-term API key 는 발급 region 에 bind 되어 있어, 호출 endpoint
        region 이 토큰 region 과 일치해야 403/invalid-key 를 피할 수 있다.
        """
        import base64
        import re
        import urllib.parse
        ...
        try:
            ...
            m = re.search(r"X-Amz-Credential=[^/]+/[^/]+/([^/]+)/bedrock/", decoded)
            if m:
                return m.group(1)
        except Exception:
            pass
    ...
    async def _get_client(self):
        ...
        .env 에 AWS_BEARER_TOKEN_BEDROCK 가 설정되어 있으면 signature_version=bearer
        로 boto3 client 를 새로 빌드해 Bearer token 으로 Bedrock 을 호출한다.
        토큰에 포함된 region 을 우선 사용한다.
        """
        if self._bearer_token:
            if self._bearer_client is None:
                ...
                bearer_config = Config(signature_version="bearer")
                if self._boto_config is not None:
                    bearer_config = self._boto_config.merge(bearer_config)
                ...
                session._components.register_component(
                    "token_provider", _BearerTokenProvider(self._bearer_token)
                )
                bearer_region = self._extract_bearer_token_region(self._bearer_token)
                region_name = bearer_region or self._region_name
                self._bearer_client = session.create_client(
                    "bedrock-runtime",
