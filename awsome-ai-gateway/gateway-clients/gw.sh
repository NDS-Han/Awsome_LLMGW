...
export GW_URL="http://host.docker.internal:8000"
export ADMIN_URL="http://host.docker.internal:8000"
...
PY
  ...
  build)
    ...
    echo "[build] vk-box ...";     "$ENGINE" build -f "$HERE/vk-box/Dockerfile" -t vk-box "$HERE/.."
    echo "✅ 빌드 완료 (claude-box, codex-box, vk-box)"
    ;;

  vk)
    # vk-box 컨테이너에서 api_key_helper 를 실행해 VK 발급.
    # OIDC 모드: host 의 ~/.gateway-cli/oidc-tokens.json 를 마운트 (gateway-cli login 사전 필요)
    # STS 모드 : host 의 ~/.aws/sso/cache 를 마운트 (aws sso login 사전 필요)
    # OIDC 환경변수가 placeholder 그대로면 STS 모드로 강제한다.
    if [[ "$OIDC_ISSUER_URL" == *"XXXXXXXXX"* || "$OIDC_CLIENT_ID" == *"COGNITO_APP_CLIENT_ID"* ]]; then
      AUTH_MODE=sts
      OIDC_ENVS=()
    else
      AUTH_MODE=oidc
      OIDC_ENVS=(-e OIDC_ISSUER_URL="$OIDC_ISSUER_URL" -e OIDC_CLIENT_ID="$OIDC_CLIENT_ID")
    fi
    # OIDC_ENVS 가 비어 있을 때 set -u 에서 unbound variable 오류를 피하기 위해 잠시 비활성화
    set +u
    VK="$("$ENGINE" run --rm -i \
      "${OIDC_ENVS[@]}" \
      -e HOME=/home/appuser \
      -v "$HOME/.aws:/home/appuser/.aws:ro" \
      -v "$HOME/.gateway-cli:/home/appuser/.gateway-cli" \
      vk-box --gateway-url "$ADMIN_URL" --auth-mode "$AUTH_MODE")"
    set -u
